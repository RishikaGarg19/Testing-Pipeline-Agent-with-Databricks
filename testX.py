...
def add_age_and_drinking_status(df: DataFrame) -> DataFrame:
    ...
    The minimum drinking age is set to 21.
    ...
    # Add a 'canDrink' column based on the age.
    # The drinking age is considered to be 21 or older.
    df_with_drinking_status = df_with_age.withColumn(
        "canDrink",
        F.when(F.col("age") >= 21, True).otherwise(False).cast(BooleanType())
    )
    
    return df_with_drinking_status
...
def main():
    ...
    """
    app_name = "CustomerDrinkingStatusPipeline"
    ...
    try:
        ...
        # Apply the transformation to add age and drinking status.
